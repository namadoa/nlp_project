...
from graph.topics.cambiario.graph import app as cambiario_app
...
st.markdown("""
Esta sección le permite realizar consultas específicas sobre el Régimen Cambiario colombiano.
La base de conocimiento incluye conceptos de la Dian sobre el régimen cambiario desde 2017 hasta 2024. Contiene 389 conceptos de la Dian sobre régimen cambiario.
...
try:
    ...
    index_name = "cambiario"  # Índice específico para cambiario
    ...
    if not pinecone_api_key:
        ...
        # Si no existe el índice específico de cambiario, usar el índice general
        if index_name not in existing_indexes:
            index_name = os.environ.get("PINECONE_INDEX_NAME", "ejhr")
            st.info(f"Usando el índice general {index_name} para consultas de Régimen Cambiario.")
        ...
        if query:
            ...
            with st.chat_message("assistant"):
                ...
                # Consultar directamente a Pinecone para Cambiario con reranking
                try:
                    # Consultar documentos específicos de Cambiario con reranking
                    ...
                    if not documents:
                        ...
                        update_flow("✍️ Generando respuesta especializada en Régimen Cambiario...")
                        time.sleep(0.5)
                        ...
                        update_flow("✨ Respuesta sobre Régimen Cambiario generada con éxito!")
                    ...
                    update_flow(f"❌ Error al consultar documentos de Régimen Cambiario o generar respuesta: {str(e)}")
...
with st.sidebar:
    st.header("Sobre el Régimen Cambiario")
    st.markdown("""
    El Régimen Cambiario de Colombia comprende las normas que regulan las operaciones de cambio de divisas y el mercado cambiario nacional.
    
    **Aspectos clave:**
    - Operaciones de obligatoria canalización
    - Reglamentación del mercado cambiario
    - Declaración de divisas y operaciones internacionales
    - Infracciones y sanciones cambiarias
    
    Consulta en esta sección todo lo relacionado con el manejo de divisas y operaciones internacionales conforme a la normativa vigente.
    """)
    
    st.divider()
    
    st.write("**Base de conocimiento**")
    st.write("• 389 conceptos especializados")
