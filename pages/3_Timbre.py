...
from graph.topics.timbre.graph import app as timbre_app
...
st.set_page_config(
    page_title="Consultas sobre Impuesto de Timbre",
    page_icon="🧾",
    layout="wide"
)
...
La base de conocimiento incluye conceptos de la Dian sobre timbre desde 2017 hasta 2024. Contiene 524 conceptos de la Dian sobre timbre.
...
try:
    ...
    index_name = "timbre"  # Índice específico para timbre
    ...
    if not pinecone_api_key:
        ...
        # Si no existe el índice específico de timbre, usar el índice general
        if index_name not in existing_indexes:
            index_name = os.environ.get("PINECONE_INDEX_NAME", "ejhr")
            st.info(f"Usando el índice general {index_name} para consultas de Timbre.")
        ...
        if query:
            ...
            with st.chat_message("assistant"):
                ...
                update_flow(f"🔄 Iniciando procesamiento de la consulta sobre Impuesto de Timbre...")
                time.sleep(0.5)
                ...
                # Consultar directamente a Pinecone para Timbre con reranking
                try:
                    # Consultar documentos específicos de Timbre con reranking
                    ...
                    if not documents:
                        ...
                        response = "Lo siento, no encontré información relevante sobre tu consulta en la base de conocimiento de Impuesto de Timbre. Por favor, intenta reformular tu pregunta o consulta otra base de conocimiento."
                        final_flow = '\n'.join(flow_steps)
                        ...
                        update_flow("✍️ Generando respuesta especializada en Impuesto de Timbre...")
                        time.sleep(0.5)
                        ...
                        update_flow("✨ Respuesta sobre Impuesto de Timbre generada con éxito!")
                    ...
                    update_flow(f"❌ Error al consultar documentos de Timbre o generar respuesta: {str(e)}")
...
with st.sidebar:
    st.header("Sobre el Impuesto de Timbre")
    st.markdown("""
    El impuesto de timbre es un tributo documental que grava la formalización de ciertos actos jurídicos realizados en Colombia o que vayan a ejecutarse en el país.
    
    **Aspectos clave:**
    - Grava documentos y actos jurídicos específicos
    - Aplicable a contratos y documentos que superen ciertos montos
    - Es un impuesto de carácter nacional
    
    Consulta en esta sección todo lo relacionado con la regulación, excepciones y aplicación de este impuesto.
    """)
    
    st.divider()
    
    st.write("**Base de conocimiento**")
    st.write("• 524 conceptos especializados")
