...
from graph.topics.aduanas.graph import app as aduanas_app
...
# Título de la página
st.title("Consultas sobre Régimen Aduanero")

# Descripción de la página
st.markdown("""
Esta sección le permite realizar consultas específicas sobre el Régimen Aduanero colombiano.
La base de conocimiento incluye conceptos de la Dian sobre aduanas desde 2017 hasta 2024. Contiene 612 conceptos de la Dian sobre régimen aduanero.
...
try:
    ...
    index_name = "aduanas"  # Índice específico para aduanas
    ...
    if not pinecone_api_key:
        ...
        # Si no existe el índice específico de aduanas, usar el índice general
        if index_name not in existing_indexes:
            index_name = os.environ.get("PINECONE_INDEX_NAME", "ejhr")
            st.info(f"Usando el índice general {index_name} para consultas de Aduanas.")
        ...
        query = st.chat_input("Escribe tu consulta sobre Régimen Aduanero...")
        ...
        if query:
            ...
            with st.chat_message("assistant"):
                ...
                update_flow(f"🔄 Iniciando procesamiento de la consulta sobre Régimen Aduanero...")
                time.sleep(0.5)
                ...
                # Consultar directamente a Pinecone para Aduanas con reranking
                try:
                    # Consultar documentos específicos de Aduanas con reranking
                    ...
                    if not documents:
                        ...
                        response = "Lo siento, no encontré información relevante sobre tu consulta en la base de conocimiento de Régimen Aduanero. Por favor, intenta reformular tu pregunta o consulta otra base de conocimiento."
                        final_flow = '\n'.join(flow_steps)
                        ...
                        update_flow("✍️ Generando respuesta especializada en Régimen Aduanero...")
                        time.sleep(0.5)
                        ...
                        update_flow("✨ Respuesta sobre Régimen Aduanero generada con éxito!")
                    ...
                    update_flow(f"❌ Error al consultar documentos de Aduanas o generar respuesta: {str(e)}")
...
with st.sidebar:
    st.header("Sobre el Régimen Aduanero")
    st.markdown("""
    El Régimen Aduanero de Colombia comprende el conjunto de normas que regulan el comercio exterior, importaciones y exportaciones de bienes.
    
    **Aspectos clave:**
    - Procedimientos de importación y exportación
    - Aranceles y gravámenes aduaneros
    - Zonas francas y regímenes especiales
    - Valoración aduanera y clasificación arancelaria
    
    Consulta en esta sección todo lo relacionado con los trámites, requisitos y procedimientos aduaneros.
    """)
    
    st.divider()
    
    st.write("**Base de conocimiento**")
    st.write("• 612 conceptos especializados")
