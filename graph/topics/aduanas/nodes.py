"""
Nodos específicos para el grafo de Aduanas.
"""
...
def retrieve_documents(state: GraphState) -> Dict[str, Any]:
    """
    Recupera documentos específicos de Aduanas usando reranking.
    """
    debug_print("---ADUANAS: RETRIEVE DOCUMENTS---")
    question = state["question"]
    
    # Usar reranking para mejorar la relevancia de los documentos
    debug_print(f"Consultando para: {question}")
    documents = retrieve_with_reranking(question, query_aduanas, top_k=8)
...
def generate_response(state: GraphState) -> Dict[str, Any]:
    ...
    """
    debug_print("---ADUANAS: GENERATE RESPONSE---")
    question = state["question"]
...
def verify_response(state: GraphState) -> Dict[str, str]:
    ...
    """
    debug_print("---ADUANAS: VERIFY RESPONSE---")
    question = state["question"]
    documents = state["documents"]
