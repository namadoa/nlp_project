"""
Nodos específicos para el grafo de Retención en la Fuente.
"""
...
def retrieve_documents(state: GraphState) -> Dict[str, Any]:
    """
    Recupera documentos específicos de Retención en la Fuente usando reranking.
    """
    debug_print("---RETENCION: RETRIEVE DOCUMENTS---")
    question = state["question"]
    
    # Usar reranking para mejorar la relevancia de los documentos
    debug_print(f"Consultando para: {question}")
    documents = retrieve_with_reranking(question, query_retencion, top_k=8)
...
def generate_response(state: GraphState) -> Dict[str, Any]:
    ...
    """
    debug_print("---RETENCION: GENERATE RESPONSE---")
    question = state["question"]
...
def verify_response(state: GraphState) -> Dict[str, str]:
    ...
    """
    debug_print("---RETENCION: VERIFY RESPONSE---")
    question = state["question"]
    documents = state["documents"]
